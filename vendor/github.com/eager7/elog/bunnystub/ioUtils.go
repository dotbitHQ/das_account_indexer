...
func warpWriter(w io.Writer, condition func()) io.Writer { return limitedWriter{w, condition} }
...
// NewIOWriter generate a io filter writer with given ioManager
func NewIOWriter(path string, kind RotatePolicy, ops ...Option) (IOWriter, error) {
	...
	// XXX
	if m.Concurrency() > 0 {
		writer.concurrency = int64(m.Concurrency())
		if m.Spinning() {
			writer.condition = func() {
				writer.spanningInvoke()
			}
		} else {
			var c bool
			writer.condition = func() {
	...
	// wait for condition write start
	writer.startWg.Wait()
...
// spinning invoke ask for the permission with a spinning-like behavior
func (w *fileWriter) spanningInvoke() {
	for {
		if atomic.LoadInt64(&w.concurrencyCounter) < w.concurrency {
			atomic.AddInt64(&w.concurrencyCounter, 1)
			return
...
func (w *lockFreeWriter) reopen(lastName string) {
	...
	// Do additional jobs like compressing the log file
	go func() {
		w.wg.Wait()
		if w.manager.Compress() {
